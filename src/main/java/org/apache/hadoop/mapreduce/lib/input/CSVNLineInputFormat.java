...
public class CSVNLineInputFormat extends FileInputFormat<LongWritable, List<Text>> {
   ...
   * @throws IOException
   */
  public static List<FileSplit> getSplitsForFile(FileStatus status, Configuration conf,
  ...
  public RecordReader<LongWritable, List<Text>> createRecordReader(InputSplit split,
    ...
    String quote = conf
        .get(CSVLineRecordReader.FORMAT_DELIMITER, CSVLineRecordReader.DEFAULT_DELIMITER);
    String separator = conf
        .get(CSVLineRecordReader.FORMAT_SEPARATOR, CSVLineRecordReader.DEFAULT_SEPARATOR);
